...
class SPEObject(object):
    ...
    def __init__(self, name, basePtr, varPositions):
        '''
        This is the SPEObject constructor. DO NOT try to instantiate one of these
        directly. SPE will automatically parse and add attributes to this class
        for use with your datatype. Use spe.makeObject instead.
        '''
        #self.base = basePtr
        #self.name = name
        #self.varlist = {}
        ...
        for var in varPositions['typeinfo']:
            ...
            if var['type'] in ['i', 'f', 'b', 'p']:
                self.varlist[var['name']] = {'addr':tmpBase, 'type':var['type']}
                tmpBase += 4
    ...
    def __setattr__(self, name, value):
        ...
        if name in self.varlist:
            ...
            if type(val).__name__ == 'dict':
                ...
                raise UnsupportedOperation('SPE currently does not support setting \
                    SPEObject instances.')
...
class CSPEManager(object):
    ...
    def parseTypesINI(self, path):
        ...
        for section in INI:
            ...
            for key in INI[section]:
                ...
                if self.Types.has_key(shortname):
                    continue
                ...
                if len(types) != len(names):
                    raise MismatchedTypesAndNames('The name string and type string don\'t line up!')
                ...
                for name, type in zip(names, types):
                    ...
                    # We need to figure out the offset of each variable. If the
                    # current variable is a native type, we can do this immediately.
                    ...
                    # Does it exist in the dictionary?
                        ...
                        if type in self.Types:
                            ...
                            raise UndefinedTypeError('Type of name %s not found in dictionary! Did you define it?' % type)
    ...
    def parseINI(self, path):
        ...
        for section in INI:
            ...
            for key in INI[section]:
                ...
                if self.Signatures.has_key(INI[section]['shortname']):
                    continue
    ...
    def detourFunction(self, functionName, type, callback):
        ...
        if not self.Signatures.has_key(functionName):
        ...
        # Hook the function
        hookFunction(sigObj.function, sigObj.param_format, convIdx, type, callback)
    ...
    def makeObject(self, objectName, baseAddress):
        ...
        if objectName in self.Types:
            ...
            raise UndefinedTypeError('Type %s not registered with SPE!' % objectName)
