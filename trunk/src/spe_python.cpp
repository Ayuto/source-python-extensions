...
bool EnablePython( void )
{
    ...
    {
        Msg("[SPE] Could not load one or more required dependencies to initialize python!\n");
        return false;
    }
#else
    ...
    // Import the Python spe module, so spe_version is set
    PyObject* spe = PyImport_ImportModule("spe");
    if (!spe)
    {
        Msg("[SPE] Could not import the Python spe module.\n");
        return false;
    }
